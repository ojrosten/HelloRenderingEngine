...
namespace curlew {
    ...
    [[nodiscard]]
    std::string get_object_label(agl::object_identifier identifier, const agl::resource_handle& handle) {
        std::string label(get_max_label_length(), ' ');
        GLsizei numChars{};
        agl::gl_function{glGetObjectLabel}(agl::to_gl_enum(identifier),
                                           handle.index(),
