...
namespace curlew {
    namespace {
        ...
        void set_debug_context(const agl::debugging_mode mode, const avocet::opengl::opengl_version& version) {
    ...
    window::window(const window_config& config, const agl::opengl_version& version)
        ...
        , m_Context{
            ...
            config.prologue,
            config.epilogue,
            [&win = m_Window](GladGLContext ctx) { load_gl_fuctions(win, ctx); return ctx; }
        }
    {
