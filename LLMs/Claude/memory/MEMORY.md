...
- [CI output-drift detection design](sequoia/project_ci_output_drift.md) — versioned-output defects show as git diffs not failures; discriminators classify leak vs stale fork; sequoia-native drift report recommended; Windows leg needs E4
- [Sanitizer axis + prune workflow + dependency-analyser contract](sequoia/project_sanitizer_prune_workflows.md) — env-slot mixins for TSan/UBSan; prune = build target, never a configure axis; cpp↔hpp pairing precondition + planned self-include warning
