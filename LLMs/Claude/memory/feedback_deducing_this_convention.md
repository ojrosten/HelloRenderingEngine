...
- `generic_buffer_object::extract_data`, `generic_texture_2d::extract_data`, `framebuffer_object::extract_data` — deducing-this. (TODO: are these read-only queries that the convention would actually classify as plain `const`? They use `glGetBufferSubData`/`glGetTexImage`/`glReadPixels` — pure queries. May be inconsistent with the refined rule, or there may be transient state changes via binding I'm not seeing. Worth verifying next time it comes up.)
