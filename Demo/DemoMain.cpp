...
public:
    ...
    [[nodiscard]]
    explicit operator GLuint() const { return m_Index; }
};
...
public:
    ...
    [[nodiscard]]
    const resource_handle& handle() const { return m_Handle.handle(); }
};
...
int main()
{
    try
    {
        ...
        glAttachShader(shaderProgram, vertexShader.handle().index());
        glAttachShader(shaderProgram, fragmentShader.handle().index());
