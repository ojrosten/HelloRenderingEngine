...
namespace avocet::opengl {
    ...
    template<class R, class... Args>
    using glad_ctx_ptr_to_mem_fn_ptr = function_pointer_type<R, Args...> GladGLContext::*;

    template<class R, class... Args, debugging_mode Mode>
    class [[nodiscard]] gl_function<R(Args...), Mode> {
    public:
        using pointer_to_member_type = glad_ctx_ptr_to_mem_fn_ptr<R, Args...>;

        constexpr static num_messages max_reported_messages{10};

        gl_function(pointer_to_member_type pm)
            : m_Fn{pm}
        {}

        gl_function(unchecked_debug_output_t, pointer_to_member_type pm)
            : gl_function{pm}
        {
    ...
    template<class R, class... Args>
    gl_function(glad_ctx_ptr_to_mem_fn_ptr<R, Args...>) -> gl_function<R(Args...)>;

    template<class R, class...Args>
    gl_function(unchecked_debug_output_t, glad_ctx_ptr_to_mem_fn_ptr<R, Args...>) -> gl_function<R(Args...), debugging_mode::none>;
}