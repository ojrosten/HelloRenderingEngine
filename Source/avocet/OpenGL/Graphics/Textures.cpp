...
namespace avocet::opengl {
    ...
    void texture_2d_lifecycle_events::configure(const contextual_resource& h, const configurator& config) {
    ...
    unique_image texture_2d::do_extract_data(const texture_2d& tex2d, texture_format format, alignment rowAlignment) {
        ...
        const auto width{static_cast<std::size_t>(extract_texture_2d_param(tex2d.context(), GL_TEXTURE_WIDTH))},
                  height{static_cast<std::size_t>(extract_texture_2d_param(tex2d.context(), GL_TEXTURE_HEIGHT))};
