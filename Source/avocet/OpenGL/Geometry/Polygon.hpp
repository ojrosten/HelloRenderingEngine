...
namespace avocet::opengl {
    ...
    public:
        ...
        polygon_base(const extended_context& ctx, Fn transformer, const std::optional<std::string>& label)
        ...
        template<class Fn>
            requires std::is_invocable_r_v<vertices_type, Fn, vertices_type> && is_textured_v
        polygon_base(const extended_context& ctx, Fn transformer, const texture_2d_configurator& texConfig, const std::optional<std::string>& label)
            :     m_VBO{ctx, transformer(st_Vertices), label}
            ,     m_VAO{ctx, label, m_VBO}
    ...
    public:
        ...
        template<class Fn>
            requires std::is_invocable_r_v<vertices_type, Fn, vertices_type> && (!is_textured_v)
        polygon(const extended_context& ctx, Fn transformer, const std::optional<std::string>& label)
        ...
        template<class Fn>
            requires std::is_invocable_r_v<vertices_type, Fn, vertices_type> && is_textured_v
        polygon(const extended_context& ctx, Fn transformer, const texture_2d_configurator& texConfig, const std::optional<std::string>& label)
    ...
    private:
        ...
        static void do_draw(const extended_context& ctx) {
