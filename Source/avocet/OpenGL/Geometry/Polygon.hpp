...
namespace avocet::opengl {
    ...
    template<std::floating_point T>
    struct make_polygon_attribute<sequoia::maths::vec_coords<T, 2, texture_arena>>{
        [[nodiscard]]
        constexpr texture_coordinates<T> operator()(std::size_t i, std::size_t N) {
    ...
    public:
        ...
        template<class Fn>
            requires std::is_invocable_r_v<vertices_type, Fn, vertices_type> && is_textured_v
        polygon(Fn transformer, const texture_2d_configurator& config, const std::optional<std::string>& label)
            : polygon_base_type{transformer, config, label}
            , m_EBO{st_Indices, label}
        {
        }
    private:
