...
namespace avocet::opengl {
    ...
    public:
        ...
        [[nodiscard]]
        R operator()(this const gl_function& self, const decorated_context_base& ctx, Args... args, std::source_location loc = std::source_location::current()) {
            return self.invoke(ctx, ctx.debug_mode(), args..., loc);
        }

        [[nodiscard]]
        R operator()(this const gl_function& self, const decorated_context_base& ctx, debugging_mode_off_type, Args... args, std::source_location loc = std::source_location::current()) {
    ...
    private:
        ...
        [[nodiscard]]
        R invoke(this const gl_function& self, const decorated_context_base& ctx, debugging_mode mode, Args... args, std::source_location loc = std::source_location::current()) {
