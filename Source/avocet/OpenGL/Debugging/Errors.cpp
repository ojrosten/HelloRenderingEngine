...
namespace avocet::opengl {
    ...
    namespace {
        ...
        [[nodiscard]]
        GLint get_max_message_length(GladGLContext& ctx, std::source_location loc) {
            GLint maxLen{};
            gl_function{unchecked_debug_output, ctx.GetIntegerv, loc}(GL_MAX_DEBUG_MESSAGE_LENGTH, &maxLen);
            return maxLen;
        }
        ...
        [[nodiscard]]
        std::optional<debug_info> get_next_message(GladGLContext& ctx, std::source_location loc) {
            ...
            const auto numFound{gl_function{unchecked_debug_output, ctx.GetDebugMessageLog}(1, to_gl_sizei(message.size()), &source, &type, &id, &severity, &length, message.data())};
        ...
        [[nodiscard]]
        error_code get_error(GladGLContext& ctx) { return error_code{gl_function{unchecked_debug_output, ctx.GetError}()}; }

#ifndef __clang__
        [[nodiscard]]
        STD_GENERATOR<error_code> get_errors(GladGLContext& ctx, num_messages maxNum) {
        ...
        [[nodiscard]]
        STD_GENERATOR<debug_info> get_messages(GladGLContext& ctx, num_messages maxNum, std::source_location loc) {
            for([[maybe_unused]] auto _ : std::views::iota(0u, maxNum.value)) {
...
#else
        [[nodiscard]]
        std::vector<error_code> get_errors(GladGLContext& ctx, num_messages maxNum) {
        ...
        [[nodiscard]]
        std::vector<debug_info> get_messages(GladGLContext& ctx, num_messages maxNum, std::source_location loc) {
