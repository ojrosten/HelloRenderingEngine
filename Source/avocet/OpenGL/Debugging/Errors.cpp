...
namespace avocet::opengl {
    ...
    namespace {
        [[nodiscard]]
        GLint get_max_message_length(const decorated_context_base& ctx) {
            GLint maxLen{};
        ...
        [[nodiscard]]
        std::optional<debug_info> get_next_message(const decorated_context_base& ctx) {
        ...
        [[nodiscard]]
        error_code get_error(const decorated_context_base& ctx) { return error_code{gl_function{&GladGLContext::GetError}(ctx, debugging_mode_off)}; }
    ...
    std::string compose_error_message(std::string_view errorMessage, const error_message_info& info) {
        return std::format("OpenGL error detected following call to {} originating from {}:\n{}\n", info.fn_name, avocet::opengl::to_string(info.loc), errorMessage);
    }
