...
namespace avocet::opengl {
    ...
    namespace {
        ...
        private:
            ...
            GLint get_parameter_value(GLenum paramName) const {
                GLint param{};
                m_ParamGetter(m_Handle.context(), m_Handle.handle().index(), paramName, &param);
                return param;
            }
            ...
            std::string get_info_log() const {
                ...
                m_InfoLogGetter(m_Handle.context(), m_Handle.handle().index(), logLen, nullptr, info.data());
                return info;
            }
        };
        ...
        public:
            ...
            [[nodiscard]]
            contextual_resource_handle create(const GladGLContext& ctx) { return contextual_resource_handle{ctx, resource_handle{gl_function{&GladGLContext::CreateShader}(ctx, to_gl_enum(m_Species))}}; }

            static void destroy(const contextual_resource_handle& handle) { gl_function{&GladGLContext::DeleteShader}(handle.context(), handle.handle().index()); }
        };
        ...
        public:
            ...
            {
                const auto index{m_Resource.handle().handle().index()};
                const auto source{read_to_string(sourceFile)};
                const auto data{source.data()};
                gl_function{&GladGLContext::ShaderSource}(m_Resource.handle().context(), index, 1, &data, nullptr);
                gl_function{&GladGLContext::CompileShader}(m_Resource.handle().context(), index);
        ...
        public:
            shader_attacher(const GladGLContext& ctx, const shader_program_resource& progResource, const shader_compiler& shader)
                : m_Context{ctx}
                , m_ProgIndex{progResource.handle().handle().index()}
                , m_ShaderIndex{shader.resource().handle().handle().index()}
            {
        ...
        : m_Resource{ctx}
    {
        ...
        const auto progIndex{m_Resource.handle().handle().index()};

        {
            shader_attacher verteAttacher{ctx, m_Resource, vertexShader}, fragmentAttacher{ctx, m_Resource, fragmentShader};
    ...
    GLint shader_program::extract_uniform_location(std::string_view name) {
        ...
        const auto location{gl_function{&GladGLContext::GetUniformLocation}(m_Resource.handle().context(), m_Resource.handle().handle().index(), name.data())};
        if(location == -1)
