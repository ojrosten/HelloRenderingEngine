...
namespace avocet::opengl {
    ...
    struct resource_lifecycle {
        ...
        static void bind(decorated_contextual_resource_view crv) { LifeEvents::bind(crv); }
    ...
    protected:
        ~generic_resource() {
            for(auto crh : contextual_handles()) {
                crh.context().utilize(LifeEvents{}, crh.handle());
            }
        }
        ...
        static void do_bind(contextual_resource_view crv) {
            crv.context().utilize(LifeEvents{}, crv.handle());
        }
 
        template<std::size_t I>
            requires (I < N)
        void do_bind(this const generic_resource& self, index<I> i) { do_bind(self.contextual_handle(i)); }
