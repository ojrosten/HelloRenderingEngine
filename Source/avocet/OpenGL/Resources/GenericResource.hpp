...
namespace avocet::opengl {
    ...
    struct resource_lifecycle {
        ...
        [[nodiscard]]
        static contextual_resource_handles<N> generate(const GladGLContext& ctx) {
            raw_indices<N> indices{};
            LifeEvents::generate(ctx, indices);
            return contextual_resource_handles<N>{ctx, indices};
        }
    ...
    public:
        ...
        [[nodiscard]]
        const contextual_resource_handles<N>& get_handles() const noexcept { return m_Handles; }
    ...
    public:
        ...
        explicit generic_resource(const GladGLContext& ctx, const std::array<configurator_type, N>& configs)
            : m_Resource{ctx}
        {
            for(const auto& [contextualHandle, config] : std::views::zip(get_handles(), configs)) {
                if(contextualHandle == contextual_resource_handle{contextualHandle.context(), {}})
                    throw std::runtime_error{"generic_resource  - null resource"};

                lifecycle_type::bind(contextualHandle);
                lifecycle_type::configure(contextualHandle, config);
            }
        }

        [[nodiscard]]
        bool is_null() const noexcept {
            auto isNull{[](const contextual_resource_handle& h) { return h == contextual_resource_handle{h.context(), {}}; }};
        ...
        template<std::size_t I>
            requires (I < N)
        [[nodiscard]]
        std::string extract_label(index<I> i) const { return get_object_label(get_handles().context(), LifeEvents::identifier, get_handle(i)); }

        [[nodiscard]]
        std::string extract_label() const requires (N == 1) { return extract_label(index<0>{}); }

        [[nodiscard]]
        const GladGLContext& context() const noexcept { return get_handles().context(); }
    ...
    private:
        [[nodiscard]]
        const contextual_resource_handles<N>& get_handles() const noexcept { return m_Resource.get_handles(); }
