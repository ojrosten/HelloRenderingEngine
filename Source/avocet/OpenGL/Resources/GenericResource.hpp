...
namespace avocet::opengl {
    ...
    struct resource_lifecycle {
        ...
        [[nodiscard]]
        static contextual_resource_handles<N> generate(const extended_context& ctx) {
    ...
    public:
        ...
        explicit resource_wrapper(const extended_context& ctx) : m_Handles{lifecycle_type::generate(ctx)} {}
    ...
    public:
        ...
        explicit generic_resource(const extended_context& ctx, const std::array<configurator_type, N>& configs)
            : m_Resource{ctx}
        {
        ...
        [[nodiscard]]
        const extended_context& context() const noexcept { return contextual_handles().context(); }
