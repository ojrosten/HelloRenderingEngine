...
namespace avocet::opengl {
    ...
    struct texture_2d_lifecycle_events {
        ...
        static void generate(const extended_context& ctx, raw_indices<N>& indices) { gl_function{&GladGLContext::GenTextures}(ctx, N, indices.data()); }

        template<std::size_t N>
        static void destroy(const extended_context& ctx, const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteTextures}(ctx, N, indices.data()); }
    ...
    public:
        ...
        explicit texture_2d(const extended_context& ctx, const configurator_type& textureConfig)
