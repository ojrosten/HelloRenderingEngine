...
namespace avocet::opengl {
    ...
    struct texture_2d_lifecycle_events {
        ...
        static void generate(const decorated_context_base& ctx, raw_indices<N>& indices) { gl_function{&GladGLContext::GenTextures}(ctx, N, indices.data()); }

        template<std::size_t N>
        static void destroy(const decorated_context_base& ctx, const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteTextures}(ctx, N, indices.data()); }
    ...
    public:
        ...
        texture_2d(const decorated_context_base& ctx, const configurator_type& textureConfig)
