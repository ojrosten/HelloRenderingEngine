    ...
    private:
        ...
        void set_attribute_ptr(attrib_ptr_info& info, std::size_t sizeofAtt, GLsizei stride) {
            ...
            if constexpr(std::is_same_v<ValueType, GLdouble>) {
                gl_function{&GladGLContext::VertexAttribLPointer}(this->context(), info.index, components, typeSpecifier, stride, (GLvoid*)info.offset);
            }
            else {
                gl_function{&GladGLContext::VertexAttribPointer}(this->context(), info.index, components, typeSpecifier, GL_FALSE, stride, (GLvoid*)info.offset);
            }
            gl_function{&GladGLContext::EnableVertexAttribArray}(this->context(), info.index);
