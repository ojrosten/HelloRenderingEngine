...
namespace avocet::opengl {
    ...
    struct vao_lifecycle_events {
        ...
        static void bind(const contextual_resource_handle& h) { gl_function{&GladGLContext::BindVertexArray}(h.context(), h.handle().index()); }
    ...
    struct buffer_lifecycle_events : common_buffer_lifecycle_events {
        ...
        static void bind(const contextual_resource_handle& h) { gl_function{&GladGLContext::BindBuffer}(h.context(), to_gl_enum(Species), h.handle().index()); }
    ...
    public:
        ...
        generic_buffer_object(const GladGLContext& ctx, std::span<const T> data, const optional_label& label)
        ...
        friend std::vector<T> extract_data(const generic_buffer_object& gbo) {
            ...
            const auto size{get_buffer_size(gbo)};
    ...
    private:
        [[nodiscard]]
        static GLint get_buffer_size(const generic_buffer_object& gbo) {
            GLint param{};
            gl_function{&GladGLContext::GetBufferParameteriv}(gbo.context(), to_gl_enum(Species), GL_BUFFER_SIZE, &param);
            return param;
        }
    };
