...
namespace avocet::opengl {
    ...
    struct vao_lifecycle_events {
        ...
        static void generate(const extended_context& ctx, raw_indices<N>& indices) { gl_function{&GladGLContext::GenVertexArrays}(ctx, N, indices.data()); }

        template<std::size_t N>
        static void destroy(const extended_context& ctx, const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteVertexArrays}(ctx, N, indices.data()); }
    ...
    struct common_buffer_lifecycle_events {
        ...
        template<std::size_t N>
        static void generate(const extended_context& ctx, raw_indices<N>& indices) { gl_function{&GladGLContext::GenBuffers}(ctx, N, indices.data()); }

        template<std::size_t N>
        static void destroy(const extended_context& ctx, const raw_indices<N>& indices) { gl_function{&GladGLContext::DeleteBuffers}(ctx, N, indices.data()); }
    };
    ...
    public:
        ...
        vertex_attribute_object(const extended_context& ctx, const optional_label& label, const vertex_buffer_object<sequoia::mem_ordered_tuple<Attributes...>>& vbo)
            : base_type{ctx, {{label}}}
        {
    ...
    public:
        ...
        generic_buffer_object(const extended_context& ctx, std::span<const T> data, const optional_label& label)
    ...
    private:
        [[nodiscard]]
        static GLint get_buffer_size(const extended_context& ctx) {
            GLint param{};
