...
namespace avocet::opengl::capabilities::impl {
    namespace {
        ...
        void do_configure(const decorated_context& ctx, face_selection_mode face, const gl_stencil_func& requested) {
            gl_function{&GladGLContext::StencilFuncSeparate}(ctx, to_gl_underlying_value<GLenum>(face), to_gl_underlying_value<GLenum>(requested.comparison), requested.reference_value, requested.mask);
        }

        void do_configure(const decorated_context& ctx, face_selection_mode face, const gl_stencil_op& requested) {
            gl_function{&GladGLContext::StencilOpSeparate}(ctx, to_gl_underlying_value<GLenum>(face), to_gl_underlying_value<GLenum>(requested.on_failure), to_gl_underlying_value<GLenum>(requested.on_pass_with_depth_failure), to_gl_underlying_value<GLenum>(requested.on_pass_without_depth_failure));
        }
