...
namespace avocet::opengl {
    template<class T>
    inline constexpr bool has_lifecycle_v{
    ...
    template<class LifeEvents>
        requires has_lifecycle_v<LifeEvents>
    ...
    struct shader_program_resource_lifecyle {
        [[nodiscard]]
        resource_handle create() { return resource_handle{glCreateProgram()}; }

        static void destroy(const resource_handle& h) { glDeleteProgram(h.index()); }
    };

    using shader_program_resource = generic_shader_resource<shader_program_resource_lifecyle>;
