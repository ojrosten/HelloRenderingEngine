...
namespace avocet::opengl {
    ...
    public:
        ...
        explicit generic_vertex_object(const std::array<configurator_type, N>& configs) {
            for(const auto&[handle, config] : std::views::zip(get_handles(), configs)) {
                if(handle == resource_handle{})
                    throw std::runtime_error{"generic_vertex_object: null resource handle"};
        ...
        template<std::size_t I>
          requires (I < N)
        [[nodiscard]]
        std::string extract_label(index<I>) const { return get_object_label(LifeEvents::identifier, get_handles()[I]); }
    ...
    protected:
        ...
        template<std::size_t I>
            requires (I < N)
        static void do_bind(const generic_vertex_object& gvo, index<I>) { lifecycle_type::bind(gvo.get_handles()[I]); }
    ...
    public:
        ...
        generic_buffer_object(std::span<const T> data, const optional_label& optLabel)
            : base_type{{{data, optLabel}}}
        {}
        ...
        friend std::vector<T> extract_data(const generic_buffer_object& gbo) {
            ...
            gl_function{glGetBufferSubData}(to_gl_enum(Species), 0, size, recoveredBuffer.data());
