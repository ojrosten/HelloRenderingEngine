...
namespace avocet::opengl {
    namespace {
        ...
        class shader_resource_lifecyle {
            shader_species m_Species;
        public:
            explicit shader_resource_lifecyle(shader_species species) : m_Species{species} {}
        ...
        using shader_resource = generic_shader_resource<shader_resource_lifecyle>;
        ...
        static_assert(has_life_events_v<shader_resource_lifecyle>);
        static_assert(has_life_events_v<shader_program_resource_lifecyle>);
    }

    shader_program::shader_program(std::string_view vertexSource, std::string_view fragmentSource)
    {
        shader_compiler vertexShader{shader_species::vertex, vertexSource}, fragmentShader{shader_species::fragment, fragmentSource};

        const auto progIndex{m_Resource.handle().index()};
        {
            shader_attacher vertexAttacher{*this, vertexShader}, fragmentAttacher{*this, fragmentShader};
