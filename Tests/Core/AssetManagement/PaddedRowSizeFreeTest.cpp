...
namespace avocet::testing
{
    namespace {
        ...
        std::size_t padded_row_size(std::size_t width, colour_channels channels, std::size_t bytesPerChannel, alignment rowAlignment) {
            ...
            if(maxVal / bytesPerChannel < channels.raw_value())
                throw std::runtime_error{std::format("padded_row_size: channels ({}) * bytesPerChannel ({}) exceeds maximum allowed value {}", channels, bytesPerChannel, maxVal)};

            const auto bytesPerTexel{bytesPerChannel * channels.raw_value()};
            if(bytesPerTexel && (maxVal / bytesPerTexel < width))
                throw std::runtime_error{std::format("padded_row_size: width ({}) * bytesPerTexek ({}) exceeds maximum allowed value {}", channels, bytesPerTexel, maxVal)};
    ...
    void padded_row_size_free_test::run_tests()
    {
        ...
        check_exception_thrown<std::runtime_error>(
            ...
            [](){ return testing::padded_row_size(0, colour_channels{0}, 0, alignment{1}); }
        );
