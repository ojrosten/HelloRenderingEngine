...
#include "ShaderProgramBuildAndVersionSelectiveFreeTest.hpp"
...
{
    [[nodiscard]]
    std::filesystem::path shader_program_build_and_version_selective_free_test::source_file() const
    {
        return std::source_location::current().file_name();
    }

    void shader_program_build_and_version_selective_free_test::run_tests()
    {
        if constexpr(!avocet::has_ndebug())
            labelling_tests();
    }

    void shader_program_build_and_version_selective_free_test::labelling_tests()
    {
