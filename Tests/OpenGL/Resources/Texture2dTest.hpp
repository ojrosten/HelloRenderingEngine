...
namespace avocet::testing
{
    ...
    private:
        ...
        void check_semantics_via_texture_data(const reporter& description, const curlew::window& w, const texture_data& first, const texture_data& second) {
            check_semantics(
                description,
                opengl::texture_2d{w.context(), opengl::texture_2d_configurator{.common_config{.decoding{first.decoding},  .parameter_setter{}, .label{ first.label}}, .data_view{image_view_over_data( first.image)}}},
        ...
        void check_semantics_via_texture_data(const reporter& description, const curlew::window& w, const texture_data& sent1, const image_data& extracted1, const texture_data& sent2, const image_data& extracted2) {
            check_semantics(
                description,
                opengl::texture_2d{w.context(), opengl::texture_2d_configurator{.common_config{.decoding{sent1.decoding}, .parameter_setter{}, .label{sent1.label}}, .data_view{image_view_over_data(sent1.image)}}},
