...
namespace avocet::opengl {
    namespace capabilities {
        ...
        namespace impl {
            bool configure(const decorated_context&, const gl_multi_sample&, const gl_multi_sample&) { return false; };

            bool configure(const decorated_context& ctx, const gl_sample_coverage& current, const gl_sample_coverage& requested) {
            ...
            bool configure(const decorated_context& ctx, const gl_blend& current, const gl_blend& requested) {
                if((requested.rgb.modes != current.rgb.modes) || (requested.alpha.modes != current.alpha.modes)) {
                ...
                if((requested.rgb.algebraic_op != current.rgb.algebraic_op) || (requested.alpha.algebraic_op != current.alpha.algebraic_op))
    ...
    class capability_manager {
        ...
        void update_config(toggled_capability<Cap>& current, const Cap& requested) {
            if(capabilities::impl::configure(m_Context, current.state, requested))
...
namespace {
    ...
    template<class... Caps>
    [[nodiscard]]
    payload_type set_payload(agl::capability_manager& capManager, const payload_type& hostPayload, const Caps&... targetCaps) {
        capManager.new_payload(hostPayload);
        capManager.new_payload(std::tuple{targetCaps...});
        return make_payload(targetCaps...);
    }
}
