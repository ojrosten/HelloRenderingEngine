...
namespace{
    ...
    struct capability_common_lifecyle {
        ...
        [[nodiscard]]
        friend bool operator==(const capability_common_lifecyle&, const capability_common_lifecyle&) noexcept = default;
    };

    namespace capabilities {
        struct gl_multi_sample : capability_common_lifecyle<gl_capability::gl_multisample> {
            ...
            [[nodiscard]]
            friend bool operator==(const gl_multi_sample&, const gl_multi_sample&) noexcept = default;
        };

        struct gl_blend : capability_common_lifecyle<gl_capability::gl_blend> {
            ...
            [[nodiscard]]
            friend bool operator==(const gl_blend&, const gl_blend&) noexcept = default;
        };
    }
    ...
    public:
        explicit capability_manager(const decorated_context& ctx)
            : m_Context{ctx}
        {
            capabilities::gl_multi_sample::disable(ctx);
        }
...
namespace avocet::testing
{
    ...
    void capability_manager_free_test::run_tests()
    {
        ...
        capManager.new_payload(std::tuple{capabilities::gl_blend{}, capabilities::gl_multi_sample{}});
        check("",  agl::gl_function{&GladGLContext::IsEnabled}(ctx, GL_BLEND));
        check("",  agl::gl_function{&GladGLContext::IsEnabled}(ctx, GL_MULTISAMPLE));
