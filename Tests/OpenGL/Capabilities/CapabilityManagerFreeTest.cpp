            ...
            void configure(const decorated_context& ctx, const gl_sample_coverage& current, const gl_sample_coverage& requested) {
                if(current != requested) {
                    gl_function{&GladGLContext::SampleCoverage}(
                        ctx, requested.coverage_val.raw_value(), static_cast<GLboolean>(requested.invert)
                    );
                }
            }
        }
...
namespace {
    ...
    payload_type set_payload(agl::capability_manager& capMan, const payload_type hostPayload, const Caps&... targetCaps) {
        ...
        auto targetPayload{make_payload(targetCaps...)};
        capMan.new_payload(targetPayload);
                            ...
                            return
                                set_payload(
                                    ...
                                    gl_blend{
                                        ...
                                        .alpha{.modes{.source{agl::blend_mode::dst_alpha},  .destination{agl::blend_mode::one_minus_dst_alpha}},  .algebraic_op{agl::blend_eqn_mode::reverse_substract}},
